<html lang="en">
  	<head>
    	<title>Nanma Store-Stock add Stock</title>
...
<?php $this->load->view('cuts/header.php'); ?>
		<div class="wrapper">
			<div class="container">
                ...
                <div class="span10 offset1">
               		<div class="well">
                    	<h3 class="header">Clear Stock</h3></div>
                <div class="well" >
		...
		success : function(data) {
				...
				$("#hide_incomplete_bill").hide();
                                ...
                                <div class="control-group">
                                  ...
                                  <div class="control-group">
									...
									<div class="controls">
										<input class="input-small" value="<?php if($value->available_stock=='' ||$value->available_stock==0||$value->available_stock==null){echo 'Out Of Stock';}else{echo number_format($value->available_stock,2,'.','');};?>" name="pdt_name" id="current_value" type="text" disabled/>
										<input class="input-small" value="<?php if($value->available_stock=='' ||$value->available_stock==0||$value->available_stock==null){echo '';}else{echo $value->stock_unit;};?>" type="text" disabled/>
										<input class="input-large" value="<?php if($value->available_stock!=''){echo $value->available_stock;}else{echo 0;};?>" name="pdt_name" id="current_value" type="hidden"/>
										</div>
								</div>
							<?php if($value->available_stock!=''&& $value->available_stock!=0){?>
								<div class="form-actions">
									<div id="hide_incomplete_bill">
                                     <?php echo anchor('stock/clear_confirm/'.$value->pdt_id,'Clear stock',array('onclick'=>"return confirm('The Stock details for this product will be set to zero. Do you want to continue ?')",'class'=>"btn btn-danger",'style'=> "margin-left:228px;"))?>
                                	  
									 
									  
									  <a class="btn" href="<?php echo site_url('/stock/clear/'); ?>">Cancel</a>
									   </div>
            ...
            <p>
              <a class="btn btn-danger" href="<?php echo site_url('stock/add')?>">Add Stock</a> <a class="btn" href="<?php echo site_url('stock/clear');?>">No Thanks</a>
            </p>
          </div>
