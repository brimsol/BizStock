...
class Sales extends MY_Controller {
	...
	function index() {
		$this -> set('title', 'Nanma :: Billing');
		$this -> set('page', 'pages/sales/search_cust_id_view');
		$this -> set('page_heading', 'Billing');
		//$this -> add('app_js', 'pages/sales/customer_id');
		$this -> render();
	}

	function verifyCustomerID($rc_num) {
		$cust_details = $this -> customer_model -> carddetails($rc_num);
		$bill_num = $this -> sales_model -> billnum();
		if($cust_details != FALSE) {
			$this -> set('rc_num', $rc_num);
			$this -> set('cust_details', $cust_details);
			$this -> set('bill_num', $bill_num);
			$this -> set('title', 'Nanma :: Billing');
			$this -> set('page', 'pages/sales/sales_view');
			$this -> set('page_heading', 'Billing');
			$this -> render();
		}
	}

	function checkForIncompleteBills() {
      $pdt_id=$this->input->post('term');
		$q_sales = "SELECT is_completed FROM sales where is_completed='N' AND pdt_id='$pdt_id'";
		$query_sales = $this -> db -> query($q_sales);
		$q_stock_audit = "SELECT is_completed FROM stock_load_audit where is_completed='N' AND pdt_id='$pdt_id'";
		$query_stock_audit = $this -> db -> query($q_stock_audit);
		if ($query_sales -> num_rows() > 0) {
			echo 'inSales';
		} if ($query_stock_audit -> num_rows() > 0) {
			echo 'inStock';
		}
	}

	function getCurrentWeekNumber() {
		$query = $this -> week_model -> getCurrentWeekNumber();
		if ($query -> num_rows() > 0) {
			$row = $query -> row();
