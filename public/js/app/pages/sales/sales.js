...
/** Auto Complete the Product ID in the Billing page **/
$("#pdt_id").autocomplete({
	...
	select : function(event, ui) {
		...
		$getProductDetails(selectedObj.value);
		$checkStock(selectedObj.value);
		$isQuotaLimited(selectedObj.value);
	},
});

$getProductDetails = function(key) {
	var k = key;
	var rc_type = 'BPL'
	$.ajax({
		url : '/index.php/product/getProductDetailsToBill/',
		dataType : 'json',
		type : 'POST',
		data : {
			term : k,
		...
		success : function(data) {
			$('#price_per_unit').val(data.pdt_sell_price + '/' + data.pdt_sell_price_unit);
			$('#price_per_unit_h').val(data.pdt_sell_price_unit);
			$('#price_per_h').val(data.pdt_sell_price);
			$('#pdt_des').val(data.pdt_des);
			$("#pdt_unit").empty();
			$populateUnitField(data.pdt_sell_price_unit);
		}
	});
}

$populateUnitField = function(key) {
	var k = key;
	if (k == 'Kg' || k == 'Gram') {
...
$checkStock = function(pdt_id) {
	...
	$.ajax({
		type : "POST",
		url : "/index.php/sales/checkForIncompleteBills/",
		data : {
			term : k,
		},
		success : function(data) {
			if (data == 'inStock') {
				...
				alert('Incomplete Sale Bill(s) found for this item. Please submit or cancel it and then continue.');
				$('#myTable :input').val("");
			}
		},
	});
...
$isQuotaLimited = function(pdt_id) {
	var k = pdt_id;
	$.ajax({
		url : '/index.php/product/isQuotaLimited/',
		dataType : 'json',
		type : 'POST',
		data : {
			term : k,
		...
		success : function(data) {
			...
			$getQuotaDetails(is_quota_limited, pdt_id);
		}
	});
}

$getQuotaDetails = function(is_quota_limited, pdt_id) {
...
$getCurrentWeek = function(pdt_id) {
	var pdt_id = pdt_id;
	$.ajax({
		url : '/index.php/sales/getCurrentWeekNumber/',
		dataType : 'json',
		type : 'POST',
		data : {
		},
		...
		complete : function() {
			$getProductQuota(pdt_id);
		}
	});
}

$getProductQuota = function(pdt_id) {
	var k = pdt_id;
	...
	alert('-----');
	$.ajax({
		url : '/index.php/quota/getProductQuota/',
		dataType : 'json',
		type : 'POST',
		data : {
			term : k,
